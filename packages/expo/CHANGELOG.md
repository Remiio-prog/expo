...
- [iOS] Fail the SwiftPM autolinking sync when two different packages declare the same pod name, instead of linking whichever copy the plugin found first. The error lists every copy and how to keep one. ([#50549](https://github.com/expo/expo/pull/50549) by [@chrfalch](https://github.com/chrfalch))
- [iOS] Fail the SwiftPM autolinking sync when a module's checked-in `Package.swift` does not export a library product for one of its pods. The error lists the products the manifest exports. ([#50549](https://github.com/expo/expo/pull/50549) by [@chrfalch](https://github.com/chrfalch))
- [iOS] Fail the SwiftPM autolinking sync when a module's checked-in `Package.swift` declares a Swift package the generated package cannot mirror, such as a local path, a registry package, module aliases, traits or a version requirement it cannot render, instead of building the module until its first import of that package fails. ([#50549](https://github.com/expo/expo/pull/50549) by [@chrfalch](https://github.com/chrfalch))
- [iOS] Fail the SwiftPM autolinking sync when an entry in the `expo-modules-autolinking prebuilt-metadata` document has a field of the wrong type, instead of guessing at it. This usually means `expo` and `expo-modules-autolinking` come from different installs. ([#50549](https://github.com/expo/expo/pull/50549) by [@chrfalch](https://github.com/chrfalch))
